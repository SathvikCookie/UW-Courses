...
// Helper to find a key/value pair in a bucket chain, and returning the
// key/value, removing it optionally
static bool FindKeyInBucket(LinkedList *bucket, HTKey_t key,
...
static bool FindKeyInBucket(LinkedList *bucket, HTKey_t key,
                            HTKeyValue_t *keyvalue, bool remove) {
  LLIterator *it = LLIterator_Allocate(bucket);
  bool found = false;
  ...
  while (LLIterator_IsValid(it)) {
    ...
    if (kv->key == key) {
      ...
      if (remove) {
        LLIterator_Remove(it, LLNoOpFree);
      }
...
bool HashTable_Insert(HashTable *table,
  ...
  if (FindKeyInBucket(chain, newkeyvalue.key, oldkeyvalue, true)) {
...
bool HashTable_Find(HashTable *table,
  ...
  return FindKeyInBucket(chain, key, keyvalue, false);
}
...
bool HashTable_Remove(HashTable *table,
  ...
  int bucket = HashKeyToBucketNum(table, key);
  LinkedList *chain = table->buckets[bucket];
  if (FindKeyInBucket(chain, key, keyvalue, true)) {
    table->num_elements -= 1;
    return true;
  }
