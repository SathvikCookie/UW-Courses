...
def vanilla_solution(x: Vector, y: Vector, A: Matrix, B: Matrix) -> Vector:
    ...
    A_A_T = [[A[i][j] + A_T[i][j] for j in range(len(A[0]))] for i in range(len(A))]
    A_Term = vanilla_matmul(A_A_T, x)

    B_Term = vanilla_matmul(B_T, y)
    result = [A_Term[i] + B_Term[i] for i in range(len(x))]
    return result
